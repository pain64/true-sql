...
public class TrueSqlTests implements TestInstanceFactory {
    ...
    @Override public Object createTestInstance(
        ...
        try {
            var uri = new URI(
                STR."file://\{System.getProperty("user.dir")}/src/test/java/net/truej/sql/compiler/A.java"
            );
            ...
            var compilationUnits = List.of(
                new SimpleJavaFileObject(uri, JavaFileObject.Kind.SOURCE) {
                    @Override public CharSequence getCharContent(
                        ...
                        return code.replace("class SuperTest", "class A extends net.truej.sql.compiler.SuperTest");
                    }
                }
            );

            var r = TestCompiler2.compile(compilationUnits).get("net.truej.sql.compiler.A");
            ...
            var theClass = new URLClassLoader(
                ...
                defineClass("net.truej.sql.compiler.A", bytes, 0, bytes.length);
            }}.loadClass(className);
