...
import net.truej.sql.util.Formatter;
...
import static net.truej.sql.util.Formatter.*;
    ...
    @TestTemplate public void test3(MainConnection cn) {
        Assertions.assertEquals(
            ...
            pretty(
                ...
                ).g.fetchList(Clinic2.class)// FIXME: rename to City
            )
        );
    }

    // FIXME: MainConnection ???
    @TestTemplate public void keepCaseInAsNames(MainDataSource ds) {
        Assertions.assertEquals(
            """
                [
                  User4[
                    userName=Joe, userInfo=null]
                  , User4[
                    userName=Donald, userInfo=Do not disturb]
                  ]
                """,
            pretty(
                ds.q("""
