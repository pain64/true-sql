...
import com.truej.sql.v3.fetch.FetcherUpdateCount;
...
public class FetcherUpdateCountTest {
    ...
    @Test void fetchUpdateCount() throws SQLException {
        Fixture.withConnection(connection -> {
            ...
            var r1 = query.fetchUpdateCount(connection, new FetcherUpdateCount.Next<>() {
            ...
            Assertions.assertThrows(
                Fail.class, () ->
                    query.fetchUpdateCount(connection, new FetcherUpdateCount.Next<>() {
                        @Override public boolean willPreparedStatementBeMoved() {
                            return false;
                        }
                        @Override public Long apply(PreparedStatement stmt) {
            ...
            var r2 = query.fetchUpdateCount(
                connection, new FetcherUpdateCount.Next<PreparedStatement>() {
                    @Override public boolean willPreparedStatementBeMoved() {
                        return true;
                    }
            ...
            Assertions.assertThrows(
                SqlExceptionR.class, () ->
                    Fixture.BAD_QUERY.fetchUpdateCount(connection, new FetcherUpdateCount.Next<>() {
                        @Override public boolean willPreparedStatementBeMoved() {
            ...
            // overload
            Assertions.assertEquals(query.fetchUpdateCount(connection).updateCount, 2);
        });
    }
                ...
                var ex = Assertions.assertThrows(
                    Fail.class, () ->
                        query.fetchUpdateCount(
                            connection, new FetcherUpdateCount.Next<>() {
                                @Override public boolean willPreparedStatementBeMoved() {
                                    return false;
                                }
                                @Override public Long apply(PreparedStatement stmt) {
